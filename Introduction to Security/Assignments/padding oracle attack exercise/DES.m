...
% converting byte representation to bit representation
bit_input = [];
bit_key = [];
for i=1:8
    bit_input = [bit_input de2bi(input64(i),8)];
end
for i=1:7
    bit_key = [bit_key de2bi(key(i),8)];
end
input64 = bit_input;
key = bit_key;
...
for i=1:8
    start_index = (i-1)*8 + 1;
    stop_index = i*8;
    byte_output(i) = bi2de(output64(start_index:stop_index));
end
output64 = byte_output;
