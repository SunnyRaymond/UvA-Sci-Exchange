...
[~, max_index] = max(key_counter);
k3_recovered = max_index - 1;

% compare it to the true k3 key specified in cipher_three()
disp(['Recovered k3 = ', num2str(k3_recovered)]); % true k3 =7

% visualize with a bar plot the counters for the k3 key guesses
bar(0:15, key_counter);
xlabel('k3 guess (0..15)');
ylabel('Counter (pairs with \Delta y = 0xC)');
title('CipherThree: counters for k3 key guesses');
grid on;
